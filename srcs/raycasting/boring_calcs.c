...
void	set_things(t_calc *c, t_data *d, int x)
{
	c->cameraX = 2 * x / (double)d->map.width - 1;
...
void	calcs(t_data *d)
{
	...
	while (x < d->map.width)
	{
		...
		c.lineHeight = (int)(d->map.height / c.perpWallDist);
		c.drawStart = -c.lineHeight / 2 + d->map.height / 2;
		if (c.drawStart < 0)
			c.drawStart = 0;
		c.drawEnd = -c.lineHeight / 2 + d->map.height / 2;
		if (c.drawEnd < 0)
			c.drawEnd = d->map.height - 1;
